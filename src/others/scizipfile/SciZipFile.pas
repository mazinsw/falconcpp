...
type
  ...
  public
    ...
    function ExtractFile(const OutFileStream: TStream): Boolean;
...
function TZipFile.ExtractFile(const OutFileStream: TStream): Boolean;
const
  BufferSize = 65536;
var
...
begin
  ...
  try {+}
    ...
    try {+}
      SetLength(S, Files[i].CommonFileHeader.UncompressedSize);
      ReadBytes := Decompressor.Read(PChar(S)^,
