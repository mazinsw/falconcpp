{
 *******************************************************************************
 *           Auxilliary Perl Compatible Regular Expressions Unit.              *
 *******************************************************************************
 * Written by Anderey V. Sorokin (see the comments below).                     *
 *******************************************************************************
 *   Modified for auxilliary PSP use by Vladimir Sibirov a.k.a. Trustmaster    *
 *   http://www.psp.furtopia.org                                               *
 *   mailto:psp@furtopia.org                                                   *
 *******************************************************************************
 * [PSP 1.4.0 - 20.08.05 - Trustmaster]:                                       *
 * - hacking this unit here and there for OBJFPC mode compatibility. No more   *
 * InvertCase property, but who cares? Instead I have tuned Split method and   *
 * reduced final executable size by about 160kb. And, oh, so tired of fighting *
 * against bugs in FPC 2.0.0. At that I hope I won't have to fight them back   *
 * with future FPC versions.                                                   *
 *******************************************************************************
}
unit regexp_tregexpr;
...
{$EXTENDEDSYNTAX ON}
//{$LONGSTRINGS ON}
{$IFNDEF FPC}{$OPTIMIZATION ON}{$ENDIF}
{$IFDEF D6}
...
{$MODE OBJFPC}{$H+}{$R+}{$Q+}{$CHECKPOINTER ON}
{$ENDIF}
...
{$ENDIF}
  ...
  public
    ...
    //modified here - falcon c++
    function Exec(const AExpression, AInputString: RegExprString): boolean;
...
{$ENDIF}
    ...
    procedure Split(AInputStr: RegExprString; var APieces: StrArray);
...
{$ENDIF}
    ...
    //property InvertCaseFunction : TRegExprInvertCaseFunction read fInvertCase write fInvertCase; //##0.935
    // Set this property if you want to override case-insensitive functionality.
    // Create set it to TRegExpr.InvertCaseFunction (InvertCaseFunction by default)
...
//const
  //TRegExpr.InvertCaseFunction : TRegExprInvertCaseFunction = TRegExpr.InvertCaseFunction;
  // defaul for InvertCaseFunction property
...
begin
  ...
  if Copy(S1, 1, MaxLen) > Copy(S2, 1, MaxLen)
    then
    Result := 1
  else if Copy(S1, 1, MaxLen) < Copy(S2, 1, MaxLen)
    then
    Result := -1
  else
    Result := 0;
...
begin
 ...
 // fInvertCase := TRegExpr.InvertCaseFunction; //###0.927
...
begin
  ...
  begin
    ...
    inc(regsize, REOpSz + RENextOffSz); // compute code size without code generation
...
{$ENDIF}
    ...
    // absence of others.
      ...
      begin
        ...
        begin
          if (PREOp(scan)^ = EXACTLY)
            and (longword(strlen(scan + REOpSz + RENextOffSz)) >= len) then
          begin
...
{$IFDEF UseSetOfChar}
    ...
    begin
      ...
      if RangeIsCI
        then
        EmitRangeCPrim(InvertCaseFunction(b)); //###0.930
    end
    else
    begin
{$IFDEF UseAsserts}
...
//          flagp := flagp or HASWIDTH or SIMPLE;
    ...
    '[':
      begin
        ...
        else if (fCompModifiers and MaskModI) <> 0
          then
...
begin
  while (s^ <> #0) and (s^ <> ch) and (s^ <> InvertCaseFunction(ch))
    do
    inc(s);
  if s^ <> #0
    then
...
//      Ch := opnd^; // store in register //###0.931
        while (Result < TheMax) and (opnd^ = scan^) do
        begin // prevent unneeded InvertCaseFunction //###0.931
          inc(Result);
          inc(scan);
        end;
        if Result < TheMax then
        begin //###0.931
          InvCh := InvertCaseFunction(opnd^); // store in register
        ...
        repeat
          ...
          begin
            if (scan >= fInputEnd) or
              ((scan^ <> opnd^) and (scan^ <> InvertCaseFunction(opnd^)))
              then
              EXIT;
            inc(scan);
            inc(opnd);
...
{$ENDIF}
      EXACTLYCI:
        begin
          ...
          if (opnd^ <> reginput^)
            and (InvertCaseFunction(opnd^) <> reginput^)
            then
            EXIT;
          len := strlen(opnd);
            //###0.929 begin
          ...
          begin
            ...
            inc(opnd);
            if (opnd^ <> save^)
              and (InvertCaseFunction(opnd^) <> save^)
              then
              EXIT;
            dec(no);
          end;
        ...
        begin //###0.936
          ...
          begin
            if (save >= fInputEnd) or
              ((save^ <> opnd^) and (save^ <> InvertCaseFunction(opnd^)))
              then
              EXIT;
            inc(save);
            inc(opnd);
    ...
    case PREOp(scan)^ of
      ...
      EXACTLYCI:
        begin
          ...
          Include(FirstCharSet, InvertCaseFunction((scan + REOpSz + RENextOffSz)^));
          EXIT;
        end;
      EXACTLY:
        begin
...
function TRegExpr.Exec(const AExpression, AInputString: RegExprString): boolean;
begin
...
procedure TRegExpr.Split(AInputStr: RegExprString; var APieces: StrArray);
var
  PrevPos: integer;
begin
  PrevPos := 1;
  if Exec(AInputStr) then
    ...
    //APieces.Add (System.Copy (AInputStr, PrevPos, MatchPos [0] - PrevPos));
      ...
      APieces[length(APieces) - 1] := System.Copy(AInputStr, PrevPos, MatchPos[0] - PrevPos);
      PrevPos := MatchPos[0] + MatchLen[0];
    until not ExecNext;
  //APieces.Add (System.Copy (AInputStr, PrevPos, MaxInt)); // Tail
  SetLength(APieces, length(APieces) + 1);
  APieces[length(APieces) - 1] := System.Copy(AInputStr, PrevPos, MaxInt);
end;
 { of procedure TRegExpr.Split
--------------------------------------------------------------}
