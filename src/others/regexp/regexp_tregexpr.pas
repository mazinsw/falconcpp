 ...
 {$ENDIF}
  ...
  TRegExpr = class
   ...
   public
    ...
    function Exec (const AExpression, AInputString: RegExprString) : boolean; {$IFDEF OverMeth} overload;
    {$ENDIF}
  ...
  if PREOp (regnext (scan))^ = EEND then begin // Only one top-level choice.
    ...
    if (flags and SPSTART) <> 0 then begin
        ...
        while scan <> nil do begin
          if (PREOp (scan)^ = EXACTLY)
             and (longword(strlen(scan + REOpSz + RENextOffSz)) >= len) then begin
  ...
  begin
   ...
   {$IFDEF UseSetOfChar}
    if b <> #0 then begin
       ...
       if RangeIsCI
        then EmitRangeCPrim (InvertCaseFunction (b)); //###0.930
      end
     else begin
       {$IFDEF UseAsserts}
...
//      Ch := opnd^; // store in register //###0.931
      while (Result < TheMax) and (opnd^ = scan^) do begin // prevent unneeded InvertCaseFunction //###0.931
        inc (Result);
        inc (scan);
       end;
